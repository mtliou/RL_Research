...
x_train_batches = torch.split(x_train_sequences, batch_size)  # Split into batches
noisy_batches = torch.split(noisy_input, batch_size)

#basic LSTM
class LSTMRecallModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers):
        super(LSTMRecallModel, self).__init__()
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)
        self.fc = nn.Linear(hidden_size, input_size)  # Project hidden state back to input size

    def forward(self, x):
        # LSTM expects input in shape [batch, seq_len, input_size]
        out, _ = self.lstm(x)  # LSTM returns output and (hidden_state, cell_state)
        # Reconstruct the entire sequence
        reconstructed = self.fc(out)
        return reconstructed

# Parameters for LSTM
input_size = 1  # Size of each input tensor (6)
hidden_size = 256  # Number of LSTM units
num_layers = 3 # Number of LSTM layers
learning_rate = 0.001
num_epochs = 30

model = LSTMRecallModel(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers)
...
for epoch in range(num_epochs):
    ...
    for batch_idx in range(num_batches):
        batch_noisy = noisy_batches[batch_idx]  # Noisy input batch
        batch_clean = x_train_batches[batch_idx]  # Original input batch

        reconstructed = model(batch_noisy)  # Pass noisy input through the model
        loss = criterion(reconstructed, batch_clean)  # Compare with clean input
